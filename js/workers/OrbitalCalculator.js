// Web Worker for orbital calculations
// This runs in a separate thread to keep the main thread responsive
...
self.onmessage = function(e) {
    ...
    switch(type) {
        case 'CALCULATE_POSITIONS':
            ...
            self.postMessage({ type: 'POSITIONS_CALCULATED', results });
            break;
            
        case 'PREDICT_ORBIT':
            const prediction = predictOrbit(data);
            self.postMessage({ type: 'ORBIT_PREDICTED', prediction });
            break;
            
        case 'CALCULATE_EPHEMERIS':
            const ephemeris = calculateEphemeris(data);
            self.postMessage({ type: 'EPHEMERIS_CALCULATED', ephemeris });
            break;
    }
};
    ...
    while (currentDate <= end) {
        ...
        if (object.orbitalSpeed) {
            ...
            ephemeris.push({
                ...
                velocity: calculateVelocity(trueAnomaly, a, e, period)
            });
        }
...
function calculateVelocity(trueAnomaly, semiMajorAxis, eccentricity, period) {
    const mu = 1.32712440018e20; // Standard gravitational parameter (m³/s²)
